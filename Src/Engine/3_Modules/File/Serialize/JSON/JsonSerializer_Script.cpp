...
rapidjson::Value JsonSerializer_Script::ToJson( const void* object )
{
	...
	// add run in editor
	json.AddMember( rapidjson::StringRef( ATTR_RUN_IN_EDITOR ),
					script->RunsInEditor(),
					_al );
									  ...
									  DeserFlag flag )
{
	...
	if ( json.HasMember( ATTR_UNIQUE_ID ) ) {
		script->SetUniqueID( json[ATTR_UNIQUE_ID].GetString() );
	}
	...
	if ( json.HasMember( ATTR_RUN_IN_EDITOR ) ) {
		script->SetRunInEditor( json[ATTR_RUN_IN_EDITOR].GetBool() );
	}
